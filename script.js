...
textArea.addEventListener('keyup', () => {
    ...
    if (textArea.innerHTML) {
        typingTimer = setTimeout(ProveriGreske, 3000);
    }
});
...
function ProveriGreske() // kad se zavrsi kucanje proverava text input boxa
{
    ...
    function ProveriZaTypos(tekst) {
        ...
        {
            if (tekst[i] == '.') {
                if (tekst[i + 1].charCodeAt(0) != 32) { PodvuciCrveno(i, i + 1); } // treba razmak posle tacke
                else if (tekst[i + 2].charCodeAt(0) >= 97 && tekst[i + 2].charCodeAt(0) <= 122) { PodvuciCrveno(i + 2, i + 2); } // treba veliko slovo
        ...
        function PodvuciCrveno(index1, index2) {
            console.log("Podvuci: " + index1 + ", " + index2);
            // ubacuje span sa klasom da bi ga podvuklo crvenom
            index1 += spanBrojac * 35;
            index2 += spanBrojac * 35;
            html = html.slice(0, index1) + '<span class=underline-crven>' + html.slice(index1, index2 + 1) + '</span>' + html.slice(index2 + 1, html.length);
            //console.log(html);
            spanBrojac++;
        }
...
function PravilnaRecIliNe(rec) {
    ...
    for (let i = index, j = index; i < recnik_latinica_alfabeticki_sort.length || j >= 0; i++, j--) // mora da ga napravim da ide ka gore i ka dole od indexa početne reči
    {
        if (i < recnik_latinica_alfabeticki_sort.length) {
            ...
            if (edit == 1) {
                return recnik_latinica_alfabeticki_sort[i]
            } else if (edit < editMin) {
                editMin = edit;
                najboljiMatch = recnik_latinica_alfabeticki_sort[i]
            }
        }
        if (j >= 0) {
            edit = EditDistDP(rec, recnik_latinica_alfabeticki_sort[j]);
            if (edit == 1) {
                return recnik_latinica_alfabeticki_sort[j]
            } else if (edit < editMin) {
                editMin = edit;
                najboljiMatch = recnik_latinica_alfabeticki_sort[j]
            }
        }
